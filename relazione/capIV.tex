...
\end{figure}
Dopo aver creato le interfacce con gli elementi grafici, sia in \textit{light mode} che in \textit{dark mode}, è stato implementato un database che tiene traccia delle \textit{tab} registrate e predette, in modo da poterle consultare anche in un secondo momento senza dover richiamare l'interprete di \textit{Tensorflow Lite}.\\
\newline
Per avviare la registrazione, basta cliccare sull'immagine centrale su cui c'è un microfono. A questo punto il cellulare registra il file audio e per terminarla bisogna ricliccare sull'immagine.
\begin{figure}[H]
	\centering
...
\textbf{Problemi riscontrati:} ricavando le immagini per ogni frame di audio e successivamente facendo la \textit{predict} per ogni immagine, si ottenevano tante soluzioni quanti erano i frame.\\ Per esempio, suonando il terzo tasto della quinta corda per due volte avremmo sull'interfaccia dell'applicazione una lunga serie di \textit{3}, perchè un frame ha una durata pochi millisecondi mentre la nota ha un range di qualche secondo. La soluzione corretta sarebbe quella di avere solo due 3. \\
\newline
...
\vspace*{2ex}
\pythonexternal{./codes/coreml.py}
\vspace*{2ex}
\textbf{Soluzioni provate:}
\begin{itemize}
	...
	\vspace*{2ex}
	\pythonexternal{./codes/coremltools2.py}
	\vspace*{2ex}
